...
`lax build` rejects the Lax759944 proof package because its namespace check reports
declarations under `Lax865980Proofs`, `Turing.TM2`, `Lax865980.Ram`, and `Option` as
declarations of `Lax759944Proofs`.

Some reported declarations are extension lemmas authored by Lax759944 in an
...
the Lax759944 source that can be renamed.
...
```text
[namespace] proof declaration Turing.TM2.step.match_1.splitter does not carry the namespace prefix `Lax759944Proofs`
[namespace] proof declaration Option.getD.match_1.splitter does not carry the namespace prefix `Lax759944Proofs`
[namespace] proof declaration Lax865980.Ram.Op.value.match_1.splitter does not carry the namespace prefix `Lax759944Proofs`
[namespace] proof declaration Lax865980Proofs.Imp.Expr.evalB_mono does not carry the namespace prefix `Lax759944Proofs`
```
...
- `proofs/Lax759944Proofs/TMToRam/CanonicalLayout.lean`
- `proofs/Lax759944Proofs/TMToRam/BoundedSemantics.lean`
- `proofs/Lax759944Proofs/TMToRam/ValueBounds.lean`

Examples include `Lax865980Proofs.Imp.Expr.evalB_mono`,
`Lax865980Proofs.Imp.Com.canonicalLayout`, and
`Lax865980Proofs.Compile.Layout.bitOverhead`.

These can be moved into Lax759944-owned namespaces, but simple substitution of the
enclosing namespace is insufficient. A declaration written as `Expr.foo`
resolves `Expr` to the imported `Lax865980Proofs.Imp.Expr` namespace, so Lean still
creates the old fully qualified name. A workaround must:

1. give every extension declaration an explicitly Lax759944-owned name;
...
The following names do not occur in the Lax759944 source:

- `Turing.TM2.step.match_1.splitter`
- `Option.getD.match_1.splitter`
- `Option.isSome.match_1.splitter`
- `Lax865980.Ram.Op.value.match_1.splitter`
- `Lax865980.Ram.Instr.effect.match_1.splitter`
- `Lax865980.Ram.run.match_1.splitter`

They are generated when Lean realizes match/equation support for imported
definitions. There is no source-level name in Lax759944 to substitute. These
diagnostics therefore cannot be repaired by renaming Lax759944 declarations.
...
`Lax759944Proofs` prefix. This makes a valid source-level rename impossible for
...
- The polynomial-time equivalence theorem now has valid Lax `conclusion:`
  metadata and lives under `Lax759944Proofs`.
...
- the required Lax865980 record is currently reported as a draft, so registration
  of Lax759944 will require Lax865980 to be registered first.
