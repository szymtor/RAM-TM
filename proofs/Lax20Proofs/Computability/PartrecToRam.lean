import Lax20Proofs.Computability.PartrecNativeBounded
...
open Lax20Proofs.TMToRam Polynomial
...
theorem encodePartrecListCodes_length
    ...
    (encodePartrecListCodes consCode zeroCode oneCode xs).length =
      Lax20.BinaryWordEncoding.bitSize xs := by
  induction xs with
  | nil => rfl
  | cons a xs ih =>
      rw [encodePartrecListCodes_cons, List.length_append,
        encodePartrecNatCodes_length, ih,
        Lax20Proofs.Encoding.bitSize_cons]

theorem trList_length (xs : List ℕ) :
    (trList xs).length = Lax20.BinaryWordEncoding.bitSize xs := by
  induction xs with
  | nil => rfl
  | cons a xs ih =>
      rw [trList, List.length_append, List.length_cons, trNat_eq_bits,
        List.length_map, ih, Lax20Proofs.Encoding.bitSize_cons]
      omega

theorem length_bits_length_le_bitSize_add_one (x : List ℕ) :
    x.length.bits.length ≤ Lax20.BinaryWordEncoding.bitSize x + 1 := by
  rw [Nat.size_eq_bits_len]
  exact Nat.size_le.2
    (Lax20Proofs.Encoding.length_lt_two_pow_bitSize_add_one x)

theorem bitSize_length_cons_le (x : List ℕ) :
    Lax20.BinaryWordEncoding.bitSize (x.length :: x) ≤
      2 * Lax20.BinaryWordEncoding.bitSize x + 2 := by
...
theorem encodePartrecInputLoopCost_le (x : List ℕ) :
    encodePartrecInputLoopCost x ≤
      51 * Lax20.BinaryWordEncoding.bitSize x + 4 := by
  ...
  rw [heq]
  exact Lax20Proofs.TMToRam.encodeInputLoopCost_le x
...
theorem partrecWithInputTime_to_ramInPolynomialOverhead
    ...
    ∃ (p : Program) (wordOverhead timeOverhead : Polynomial ℕ),
      ∀ (x : List ℕ) (w : ℕ),
        wordOverhead.eval (Lax20.BinaryWordEncoding.bitSize x + time x) ≤ w →
          ∃ t ≤ timeOverhead.eval
              (Lax20.BinaryWordEncoding.bitSize x + time x),
  ...
  intro x w hw
  let n := Lax20.BinaryWordEncoding.bitSize x
  let runningTime := time x
  let q := n + runningTime
  have hrun' := hrun x
  have hprefix : Lax20.BinaryWordEncoding.bitSize (x.length :: x) ≤
      2 * n + 2 := by
    simpa [n] using bitSize_length_cons_le x
  have houtSize : Lax20.BinaryWordEncoding.bitSize (f x) ≤
