...
noncomputable def FinTM2.inputSymbolCode (tm : Turing.FinTM2)
    ...
    (a : Lax759944.BinaryWordEncoding.Symbol) : ℕ :=
...
theorem FinTM2.codeStack_map_inputAlphabet (tm : Turing.FinTM2)
    (inputAlphabet : tm.Γ tm.k₀ ≃ Lax759944.BinaryWordEncoding.Symbol)
    (word : List Lax759944.BinaryWordEncoding.Symbol)
...
theorem encodeNatCodes_eq_inputSymbolCodes (tm : Turing.FinTM2)
    ...
    encodeNatCodes
      ...
      (Lax759944.BinaryWordEncoding.encodeNat n).map
        (FinTM2.inputSymbolCode tm inputAlphabet) := by
  simp [encodeNatCodes, Lax759944.BinaryWordEncoding.encodeNat,
    boolDigitCode]
...
theorem encodeWordCodes_eq_inputSymbolCodes (tm : Turing.FinTM2)
    ...
    encodeWordCodes
      ...
      (Lax759944.BinaryWordEncoding.encode x).map
        (FinTM2.inputSymbolCode tm inputAlphabet) := by
  induction x with
  | nil => rfl
  | cons a x ih =>
      ...
      simp [Lax759944.BinaryWordEncoding.encode]
...
noncomputable def FinTM2.outputSymbolCode (tm : Turing.FinTM2)
    ...
    (a : Lax759944.BinaryWordEncoding.Symbol) : ℕ := by
  classical
...
theorem FinTM2.outputSymbolCode_eq_codeSymbol (tm : Turing.FinTM2)
    (outputAlphabet : tm.Γ tm.k₁ ≃ Lax759944.BinaryWordEncoding.Symbol)
    (a : Lax759944.BinaryWordEncoding.Symbol)
...
theorem encodeInputLoopCost_le (x : List ℕ) :
    encodeInputLoopCost x ≤ 51 * Lax759944.BinaryWordEncoding.bitSize x + 4 := by
  induction x with
  | nil => simp [encodeInputLoopCost, Lax759944.BinaryWordEncoding.bitSize]
  | cons a x ih =>
      rw [encodeInputLoopCost]
      rw [Lax759944Proofs.Encoding.bitSize_cons]
      have hlen := Lax759944Proofs.Encoding.length_le_bitSize x
      omega
