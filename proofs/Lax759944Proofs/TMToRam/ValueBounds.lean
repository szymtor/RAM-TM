...
theorem initEnv_bitBounded (ext : String → ℕ) (x : List ℕ) :
    EnvBitBounded (initEnv ext (x.length :: x))
      (Lax759944.BinaryWordEncoding.bitSize x + 1) := by
  refine ⟨?_, ?_, ?_, ?_⟩
  · intro name
    simp [initEnv]
  · intro name v hv
  ...
  · intro v hv
    ...
    · exact Lax759944Proofs.Encoding.length_lt_two_pow_bitSize_add_one x
    · exact Lax759944Proofs.Encoding.mem_lt_two_pow_bitSize_add_one hv
  · simp [initEnv]
