...
theorem FinTM2.compileOutputCodec_haltList (tm : Turing.FinTM2)
    ...
    (hfinal : StacksWithin (FinTM2.availableSymbols tm)
      (Turing.haltList tm (List.map outputAlphabet.invFun
        (Lax759944.BinaryWordEncoding.encode y))).stk)
    (hrep : NumericRep σ capacity
      (FinTM2.encodeNumericState tm
        (Turing.haltList tm (List.map outputAlphabet.invFun
          (Lax759944.BinaryWordEncoding.encode y))) hfinal))
    (hout : σ.out = []) :
    ∃ σ' cost,
      ...
      cost ≤ 29 * (Lax759944.BinaryWordEncoding.encode y).length + 19 ∧
      σ'.out = y := by
  ...
  have havail : ∀ a ∈ (Lax759944.BinaryWordEncoding.encode y).map
  ...
  have hcodes : FinTM2.codeStack tm tm.k₁
      ((Lax759944.BinaryWordEncoding.encode y).map outputAlphabet.invFun) havail =
      codes := by
    rw [FinTM2.codeStack_map_outputAlphabet tm outputAlphabet
      (Lax759944.BinaryWordEncoding.encode y) havail]
    symm
    exact encodeWordCodes_eq_outputSymbolCodes tm outputAlphabet y
  have hhaltStack :
      (Turing.haltList tm (List.map outputAlphabet.invFun
        (Lax759944.BinaryWordEncoding.encode y))).stk tm.k₁ =
        List.map outputAlphabet.invFun (Lax759944.BinaryWordEncoding.encode y) := by
  ...
  have hstackRep' : StackRep σ stack (capacity stack)
      (FinTM2.codeStack tm tm.k₁
        (List.map outputAlphabet.invFun (Lax759944.BinaryWordEncoding.encode y))
        havail) := by
  ...
  have hcodesLength : codes.length =
      (Lax759944.BinaryWordEncoding.encode y).length := by
    dsimp [codes]
